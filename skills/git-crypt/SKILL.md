...
license: "MIT"
metadata: {"version":"1.1.0","triggers":["unlock git-crypt","git-crypt unlock","解锁仓库","解锁加密文档","解密 AGENTS.md","decrypt internal docs","lock repo","锁定仓库","锁上文档","git-crypt lock","encrypt this repo","给仓库加密","上 git-crypt","encrypt internal docs","git-crypt status","加密状态"],"tags":["git-crypt","gpg","bitwarden","rbw","encryption","developer-tools"]}
---
...
| Bitwarden item (rbw) | `gpg-git-crypt` — armored private key in **notes**, key passphrase in field **`passphrase`** |
...
```bash
rbw unlock
rbw get gpg-git-crypt --field notes | gpg --import   # pipe; never write the key to a file
```
If the item is missing: `rbw list | grep -i gpg`, tell the user the expected layout (`gpg-git-crypt`, key in notes, passphrase in field `passphrase`); do not guess at other vault items.
