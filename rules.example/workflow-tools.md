...
When a skill under `skills/` is modified and committed, publish it to ClawHub:

```bash
clawhub publish ./skills/<skill-name> --slug <skill-name> --version <new-version> --changelog "<summary of changes>"
```
...
- If `clawhub` is not in PATH, skip and inform the user.
